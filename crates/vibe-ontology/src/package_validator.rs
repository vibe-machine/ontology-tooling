...
const MAX_WRITE_UNIT_CHARS: usize = 50_000;
const MAX_WRITE_UNIT_BLOCKS: usize = 50;
const APPLY_UNITS_ROOT: &str = "generated/apply-units";
...
fn validate_executable_package(repo_path: &Path, package: &Value) -> Result<()> {
    let mut checked = BTreeSet::new();
    for relative_path in string_array(package.get("data")) {
        maybe_check_write_unit(repo_path, relative_path, &mut checked)?;
    }
    for relative_path in provenance_files(package) {
        maybe_check_write_unit(repo_path, relative_path, &mut checked)?;
    }

    let schema_paths: BTreeSet<&str> = package
        .get("schemas")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
    ...
    for relative_path in string_array(
        ...
        if relative_path.ends_with(".tql") && !schema_paths.contains(relative_path) {
            maybe_check_write_unit(repo_path, relative_path, &mut checked)?;
        }
    }
...
fn provenance_files(package: &Value) -> Vec<&str> {
    match package.get("provenance") {
        Some(Value::Array(values)) => values.iter().filter_map(Value::as_str).collect(),
        Some(Value::Object(provenance)) => string_array(provenance.get("files")),
        _ => Vec::new(),
    }
}
...
fn maybe_check_write_unit(
    ...
    if !relative_path.ends_with(".tql") || !checked.insert(relative_path.to_string()) {
        return Ok(());
    }
...
fn assert_safe_write_unit(relative_path: &str, text: &str) -> Result<()> {
    ...
    let char_count = trimmed.encode_utf16().count();
...
fn split_executable_blocks(text: &str) -> Vec<String> {
    let significant: Vec<&str> = text
        .lines()
        .map(str::trim)
        .filter(|line| !line.is_empty() && !line.starts_with('#'))
        .collect();
    let has_put = significant.iter().any(|line| line.starts_with("put "));
    let has_non_put_write = significant.iter().any(|line| {
        line.starts_with("match")
            || line.starts_with("insert ")
            || line.starts_with("delete ")
            || line.starts_with("update ")
    });
    if has_put && !has_non_put_write {
        return grouped_put_blocks(text);
    }
...
fn grouped_put_blocks(text: &str) -> Vec<String> {
    ...
    for statement in statements {
        if let Some(variable) = put_entity_variable(&statement) {
            groups.push((Some(variable), vec![statement]));
        } else if let Some((Some(variable), group_statements)) = groups.last_mut() {
            if statement.contains(&format!("${variable}")) {
                group_statements.push(statement);
            } else {
                groups.push((None, vec![statement]));
            }
        } else {
            groups.push((None, vec![statement]));
        }
    ...
    groups
        .into_iter()
        .map(|(_, statements)| statements.join("\n\n"))
        .collect()
}

fn put_entity_variable(statement: &str) -> Option<String> {
    let rest = statement.strip_prefix("put ")?.trim_start();
    ...
    let remainder = rest[variable_len..].trim_start();
    if !remainder.starts_with("isa ") {
        return None;
    }
